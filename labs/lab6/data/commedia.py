...
def S1_estimateModel(hlTercets, eps=0.1):
    ...
    for cls in hlTercets:  # Loop over class labels
        ...
        for w in h_clsLogProb[cls]:  # Loop over all words
            h_clsLogProb[cls][w] = numpy.log(h_clsLogProb[cls][w]) - numpy.log(
                nWordsCls
            )  # Compute log N_{cls,w} / N
    ...
    for cls in hlTercets:  # Loop over class labels
        ...
        for tercet in lTercets:  # Loop over all tercets of the class
            ...
            for w in words:  # Loop over words of the given tercet
                ...
                h_clsLogProb[cls][
                    wordIdx
                ] += 1  # h_clsLogProb[cls] ius a 1-D array, h_clsLogProb[cls][wordIdx] is the element in position wordIdx
...
if __name__ == "__main__":
    ...
    # print("S1 - Model parameters:")
    # print(S1_model)
    # input()

    # print(f"{S1_model['inferno']['e']}")
    # input()
    ...
    S1_predictions = compute_classPosteriors(
        ...
        numpy.log(numpy.array([1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0])),
    )

    # print(S1_predictions)
    # input()
    ...
    # Per-class accuracy
    print(
        "Multiclass - S1 - Inferno - Accuracy:",
        compute_accuracy(
            S1_predictions[:, labelsEval == hCls2Idx["inferno"]],
            labelsEval[labelsEval == hCls2Idx["inferno"]],
        ),
    )
    print(
        "Multiclass - S1 - Purgatorio - Accuracy:",
        compute_accuracy(
            S1_predictions[:, labelsEval == hCls2Idx["purgatorio"]],
            labelsEval[labelsEval == hCls2Idx["purgatorio"]],
        ),
    )
    print(
        "Multiclass - S1 - Paradiso - Accuracy:",
        compute_accuracy(
            S1_predictions[:, labelsEval == hCls2Idx["paradiso"]],
    ...
    hlTercetsTrain = {"inferno": lInfTrain, "paradiso": lParTrain}
    ...
    S1_predictions = compute_classPosteriors(
        ...
        numpy.log(numpy.array([1.0 / 2.0, 1.0 / 2.0])),
    )
    ...
    print(
        "Binary [inferno vs paradiso] - S1 - Accuracy:",
    ...
    print(
        "Binary [inferno vs paradiso] - S2 - Accuracy:",
